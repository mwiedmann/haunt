...
const checkTile = (x, y) => {
  ...
  for (let yCheck = startY - radius; yCheck <= startY + radius; yCheck++) {
    for (let xCheck = startX - radius; xCheck <= startX + radius; xCheck++) {
...
for (let y = startY - radius; y <= startY + radius; y++) {
  for (let x = startX - radius; x <= startX + radius; x++) {
    ...
    if (x === startX && y === startY) {
      ...
      adjBitNum()
      output += checkTile(x, y) ? mapBaseData[y * 64 + x] : "X";
    }
  }
  output += "\n";
}
...
fs.writeFileSync(`build/PRECALC.BIN`, new Uint8Array([...finalBytes]), "binary");
